...
| Never commit weights, renders, inputs, `.env`, or caches — except the locked voice anchors | A 7 GB checkpoint in git history is effectively permanent. The anchors are the documented exception: 4.2 MB, and OmniVoice's auto mode cannot regenerate them, so ignoring them made a fresh clone unable to reproduce its own locked narration. | `.githooks/pre-commit`, `.gitignore` |
