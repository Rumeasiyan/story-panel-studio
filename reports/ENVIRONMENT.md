# Environment snapshot

- Generated: 2026-08-01T01:54:12+05:30
- Project root: `/home/sshroot/external/ai-video-gen`

## System

| Item | Value |
|---|---|
| Fedora | Fedora release 43 (Forty Three) |
| Kernel | 7.1.3-101.fc43.x86_64 |
| Session | wayland / Hyprland |
| GPU | NVIDIA GeForce RTX 3050 |
| NVIDIA driver | 580.173.02 |
| VRAM | 8192 MiB |
| RAM | 31Gi |
| Secure Boot | SecureBoot enabled |

## Software

| Item | Value |
|---|---|
| Git | 2.55.0 |
| Python (.venv) | 3.13.14 |
| pip | 26.2 |
| PyTorch | 2.13.0+cu130 |
| PyTorch CUDA runtime | 13.0 |
| torch.cuda.is_available() | True |
| FFmpeg | 7.1.5 |
| ComfyUI commit | `6cedd34343ba3214ca9591397bd106e02ef2acf6` |
| ComfyUI subject | [Partner Nodes] fix(ByteDance): encode stereo reference audio without doubling its duration (#15177) |

## Custom nodes

_none installed_

## Model profiles

```
active profile : (none)
project root   : /home/sshroot/external/ai-video-gen
free space     : 85.53 GB

  anime-sdxl       not installed
      missing                  models/checkpoints/animagine-xl-4.0-opt.safetensors (6.94 GB)
  cinematic-sdxl   not installed
      missing                  models/checkpoints/RealVisXL_V4.0.safetensors (6.94 GB)
  wan22-ti2v-5b    not installed
      missing                  models/diffusion_models/wan2.2_ti2v_5B_fp16.safetensors (10.00 GB)
      missing                  models/vae/wan2.2_vae.safetensors (1.41 GB)
      missing                  models/text_encoders/umt5_xxl_fp8_e4m3fn_scaled.safetensors (6.74 GB)

models/ on disk : ?
missing/invalid : 32.02 GB would need downloading
```

## Disk usage

```
  models                          ?
    checkpoints                   ?
    diffusion_models              ?
    text_encoders                 ?
    vae                           ?
    loras                         ?
  input                           ?
  output                          ?
  temp                            ?
  cache                           ?
  logs                     10.79 KB
  user                      6.73 MB
  .venv                     6.40 GB
  engine/ComfyUI           54.80 MB

  project total             6.55 GB
  free on volume           85.53 GB
Filesystem                                             Size  Used Avail Use% Mounted on
/dev/mapper/luks-7a83c357-2176-4a75-b6c7-d56d23ad2e94  236G  155G   80G  66% /home
```

## pip freeze

```
aiohappyeyeballs==2.7.1
aiohttp==3.14.3
aiosignal==1.4.0
alembic==1.18.5
annotated-doc==0.0.5
annotated-types==0.8.0
anyio==4.14.2
attrs==26.1.0
av==18.0.0
blake3==1.0.9
certifi==2026.7.22
cffi==2.1.0
chardet==7.4.3
charset-normalizer==3.4.9
click==8.4.2
comfy-aimdo==0.4.10
comfy-angle==0.1.0
comfy-kitchen==0.2.25
comfyui-embedded-docs==0.5.9
comfyui-manager==4.2.2
comfyui-workflow-templates-core==0.3.288
comfyui-workflow-templates-json==0.1.23
comfyui-workflow-templates-media-api==0.3.84
comfyui-workflow-templates-media-assets-01==0.1.15
comfyui-workflow-templates-media-image==0.3.160
comfyui-workflow-templates-media-other==0.3.229
comfyui-workflow-templates-media-video==0.3.101
comfyui_frontend_package==1.47.11
comfyui_workflow_templates==0.11.23
cryptography==50.0.0
cuda-bindings==13.0.3
cuda-pathfinder==1.2.2
cuda-toolkit==13.0.3.0
einops==0.8.2
filelock==3.29.0
frozenlist==1.8.0
fsspec==2026.4.0
gitdb==4.0.12
GitPython==3.1.57
greenlet==3.5.4
h11==0.16.0
hf-xet==1.5.2
httpcore==1.0.9
httpx==0.28.1
huggingface_hub==1.26.0
idna==3.18
Jinja2==3.1.6
kornia==0.8.3
kornia_rs==0.1.14
Mako==1.3.12
markdown-it-py==4.2.0
MarkupSafe==3.0.3
mdurl==0.1.2
mpmath==1.3.0
multidict==6.7.1
networkx==3.6.1
numpy==2.4.4
nvidia-cublas==13.1.1.3
nvidia-cuda-cupti==13.0.85
nvidia-cuda-nvrtc==13.0.88
nvidia-cuda-runtime==13.0.96
nvidia-cudnn-cu13==9.20.0.48
nvidia-cufft==12.0.0.61
nvidia-cufile==1.15.1.6
nvidia-curand==10.4.0.35
nvidia-cusolver==12.0.4.66
nvidia-cusparse==12.6.3.3
nvidia-cusparselt-cu13==0.8.1
nvidia-nccl-cu13==2.29.7
nvidia-nvjitlink==13.2.78
nvidia-nvshmem-cu13==3.4.5
nvidia-nvtx==13.0.85
packaging==26.2
pillow==12.2.0
propcache==0.5.2
psutil==7.2.2
pycparser==3.0
pydantic==2.13.4
pydantic-settings==2.14.2
pydantic_core==2.46.4
PyGithub==2.9.1
Pygments==2.20.0
PyJWT==2.13.0
PyNaCl==1.6.2
PyOpenGL==3.1.10
python-dotenv==1.2.2
PyYAML==6.0.3
regex==2026.7.19
requests==2.34.2
rich==15.0.0
safetensors==0.8.0
scipy==1.18.0
sentencepiece==0.2.2
setuptools==83.0.0
shellingham==1.5.4
simpleeval==1.0.7
smmap==5.0.3
spandrel==0.4.2
SQLAlchemy==2.0.51
sympy==1.14.0
tokenizers==0.22.2
toml==0.10.2
torch==2.13.0+cu130
torchaudio==2.11.0+cu130
torchsde==0.2.6
torchvision==0.28.0+cu130
tqdm==4.70.0
trampoline==0.1.2
transformers==5.14.1
triton==3.7.1
typer==0.27.0
typing-inspection==0.4.2
typing_extensions==4.15.0
urllib3==2.7.0
uv==0.12.1
wheel==0.47.0
yarl==1.24.5
```
